...
export class GuardianAI {
   ...
   */
  public validateAction(action: GuardianAction): GuardianResult {
    ...
    if (dangerousCheck.detected) {
      ...
      result.riskLevel = dangerousCheck.risk as any;
   ...
   */
  private checkDangerousActions(action: string, context: string): {
    detected: boolean;
    type: string;
    reason: string;
    risk: string;
   ...
   * Check for toxic language
   */
  private checkToxicity(action: string, context: string): {
    detected: boolean;
    score: number;
    reason: string;
  } {
    let totalScore = 0;
    let detectedToxicity: string[] = [];
   ...
   */
  private calculateConfidence(result: GuardianResult): number {
    let confidence = 1.0;
    
    // Reduce confidence based on detected issues
    if (result.metadata.biasScore > 0.5) confidence -= 0.2;
    if (result.metadata.toxicityScore > 0.5) confidence -= 0.2;
    if (result.status === 'warn') confidence -= 0.1;
   ...
   */
  public generateRecommendations(result: GuardianResult): string[] {

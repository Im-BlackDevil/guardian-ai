...
import { guardianAI } from '../guardian.js';
...
router.post('/check-action', async (req, res) => {
    try {
        ...
        const result = guardianAI.validateAction(guardianAction);
        // Create log entry
        const logEntry = {
...
router.get('/logs', async (req, res) => {
    try {
        const { page = 1, limit = 50, status, userId, sessionId, startDate, endDate, actionCategory } = req.query;
        ...
        if (actionCategory) {
            filteredLogs = filteredLogs.filter(log => log.metadata?.actionCategory === actionCategory);
        }
...
router.get('/logs/:id', async (req, res) => {
    try {
        ...
        const logEntry = guardianLogs.find(log => log.timestamp.getTime() === timestamp.getTime());
        if (!logEntry) {
        ...
        res.json({
            ...
            data: {
                ...
                recentActivity: {
                    ...
                    lastHour: guardianLogs.filter(log => log.timestamp >= new Date(Date.now() - 60 * 60 * 1000)).length
                },
                systemHealth: {
